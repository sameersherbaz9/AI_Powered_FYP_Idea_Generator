...
- Never commit a real `.env` file. Both `.env` and `backend/.env` are
  already in `.gitignore` — if you're starting from a zip/export rather
  than a fresh git clone, double-check no real `.env` file is bundled in
  before sharing the project further.
- If you ever suspect your Groq API key or Gmail App Password has leaked,
  revoke and regenerate them immediately (console.groq.com for Groq;
  Google Account → Security → App Passwords for Gmail) and update
  `backend/.env`.
